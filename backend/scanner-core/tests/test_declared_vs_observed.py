...
def _tool(name: str, *permissions: Permission) -> McpTool:
    return McpTool(
        name=name, description="", file_path="server.py", permissions=permissions
    )
...
def test_an_undeclared_observed_capability_is_upweighted_and_marked():
    ...
    reaching a shell."""
    tools = [_tool("search_files", Permission.FS_READ)]
...
def test_a_declared_capability_is_left_exactly_as_the_adapter_set_it():
    ...
    tools = [_tool("run_command", Permission.EXEC)]
...
def test_an_unknown_tool_name_is_never_guessed_at():
    tools = [_tool("other_tool", Permission.EXEC)]
...
def test_over_description_is_never_touched():
    ...
    tools = [_tool("run_command", Permission.EXEC, Permission.FS_WRITE, Permission.NETWORK)]
