...
from aevrin_api.integrations import geo
...
def _hops(monkeypatch, count: int) -> None:
    """Pretend the app sits behind `count` appending reverse proxies."""
    settings = geo.get_settings()
    monkeypatch.setattr(
        geo, "get_settings", lambda: settings.model_copy(update={"trusted_proxy_hops": count})
    )


def test_one_load_balancer_resolves_to_the_address_it_appended(monkeypatch):
    """A bare ALB appends the peer it saw, so the last entry is the client."""
    _hops(monkeypatch, 1)
    assert geo.client_ip(_request({"x-forwarded-for": "202.131.143.39"})) == "202.131.143.39"


def test_a_forged_header_cannot_choose_the_currency(monkeypatch):
    """The reason this is not read left to right. AWS ALB appends rather than
    replaces, so anything the client sent survives in front of the real
    address. Trusting it would sell Pro at Rs 1,499 instead of $34 to anyone
    who sets one header."""
    _hops(monkeypatch, 1)
    forged = "103.48.196.103"  # an Indian address, i.e. the cheaper currency
    req = _request({"x-forwarded-for": f"{forged}, 202.131.143.39"})
    assert geo.client_ip(req) == "202.131.143.39"


def test_a_second_proxy_shifts_which_entry_is_trusted(monkeypatch):
    """CloudFront in front of an ALB: CloudFront appends the client, the ALB
    appends CloudFront. Two hops back is the client."""
    _hops(monkeypatch, 2)
    req = _request({"x-forwarded-for": "103.48.196.103, 202.131.143.39, 130.176.1.1"})
    assert geo.client_ip(req) == "202.131.143.39"


def test_fewer_entries_than_configured_hops_falls_back_to_the_socket(monkeypatch):
    """A request that did not come through the expected chain. Reaching
    further left would be reading exactly the values we do not trust."""
    _hops(monkeypatch, 2)
    req = _request({"x-forwarded-for": "8.8.8.8"}, client_host="1.1.1.1")
    assert geo.client_ip(req) == "1.1.1.1"


def test_carrier_grade_nat_is_not_mistaken_for_a_public_address(monkeypatch):
    """100.64.0.0/10 is RFC 6598 space, which Python reports as neither
    private nor reserved. A hand-rolled check let it through."""
    _hops(monkeypatch, 1)
    assert geo.client_ip(_request({"x-forwarded-for": "100.64.0.6"})) is None


def test_a_garbage_trusted_entry_is_unknown_rather_than_searched(monkeypatch):
    """Deliberately not a scan for the next usable value: falling through is
    how a forged entry would get picked up."""
    _hops(monkeypatch, 1)
    assert geo.client_ip(_request({"x-forwarded-for": "8.8.8.8, not-an-ip"})) is None
...
def test_private_and_loopback_addresses_are_not_looked_up(value, monkeypatch):
...
def test_a_malformed_address_is_rejected_rather_than_sent_upstream(monkeypatch):
    _hops(monkeypatch, 1)
