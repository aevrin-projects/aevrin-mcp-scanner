...
async def test_currency_follows_the_detected_country(country, expected) -> None:
    with mock.patch("aevrin_api.routers.billing.country_for_request", return_value=country):
        assert await _resolve_currency(_request()) == expected


@pytest.mark.asyncio
async def test_an_unknown_country_resolves_to_the_more_expensive_currency() -> None:
    ...
    with mock.patch("aevrin_api.routers.billing.country_for_request", return_value=None):
        currency = await _resolve_currency(_request())
    assert currency == "USD"
    # And USD really is the dearer of the two at a realistic exchange rate.
    assert _tier_amount("pro", "monthly", "USD") / 100 * 88 > _tier_amount("pro", "monthly", "INR") / 100


@pytest.mark.asyncio
async def test_anyone_may_choose_to_pay_in_usd() -> None:
    ...
    with mock.patch("aevrin_api.routers.billing.country_for_request", return_value="IN"):
        assert await _resolve_currency(_request(), "USD") == "USD"


@pytest.mark.asyncio
async def test_inr_cannot_be_requested_from_outside_india() -> None:
    ...
    with mock.patch("aevrin_api.routers.billing.country_for_request", return_value="US"):
        assert await _resolve_currency(_request(), "INR") == "USD"


@pytest.mark.asyncio
async def test_an_indian_caller_keeps_inr_when_requesting_it() -> None:
    with mock.patch("aevrin_api.routers.billing.country_for_request", return_value="IN"):
        assert await _resolve_currency(_request(), "INR") == "INR"


@pytest.mark.asyncio
async def test_a_nonsense_currency_falls_back_to_detection() -> None:
    with mock.patch("aevrin_api.routers.billing.country_for_request", return_value="IN"):
        assert await _resolve_currency(_request(), "XYZ") == "INR"
    with mock.patch("aevrin_api.routers.billing.country_for_request", return_value=None):
        assert await _resolve_currency(_request(), "'; DROP TABLE--") == "USD"
