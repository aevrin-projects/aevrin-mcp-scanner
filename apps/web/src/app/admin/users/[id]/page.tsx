...
function GrantAddons({ detail, onDone }: { detail: AdminUserDetail; onDone: () => Promise<void> }) {
  ...
  async function grant() {
    setBusy(true);
    try {
      const result = await api.adminGrantAddon(detail.user_id, {
        addon,
        quantity,
...
function QuotaOverrides({ detail, onDone }: { detail: AdminUserDetail; onDone: () => Promise<void> }) {
  ...
  async function apply() {
    setBusy(true);
    try {
      await api.adminSetOverride(detail.user_id, {
        bucket,
  ...
  async function clear(b: string) {
    try {
      await api.adminClearOverride(detail.user_id, b);
  ...
  async function resetUsage(b: string) {
    try {
      await api.adminResetUsage(detail.user_id, { bucket: b, reason: "Support reset from admin panel" });
...
function DangerZone({ detail, onDone }: { detail: AdminUserDetail; onDone: () => Promise<void> }) {
  ...
  async function setStatus(next: "active" | "disabled" | "blocked") {
    ...
    setBusy(true);
    try {
      await api.adminSetStatus(detail.user_id, { status: next, reason, totp_code: code });
  ...
  async function sendReset() {
    setBusy(true);
    try {
      const r = await api.adminPasswordReset(detail.user_id, reason || "Support request");
