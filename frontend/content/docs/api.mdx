...
Base URL: `https://api.aevrin.net`. Every endpoint here is what the CLI, the
...
<Tabs items={['API key', 'Bearer token']}>
  <Tab value="API key">
    ...
    **Settings → API Keys** on the dashboard. Sent as a header:
    ...
    ```bash
    curl -H "X-API-Key: aevrin_xxxxx" https://api.aevrin.net/cli/precheck
    ```
  </Tab>
  <Tab value="Bearer token">
    ...
    ```bash
    curl -H "Authorization: Bearer <supabase-access-token>" https://api.aevrin.net/scans
    ```
  </Tab>
</Tabs>
