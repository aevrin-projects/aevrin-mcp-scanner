...
Next.js 16 (App Router) website: Screens 1–4 of the master build spec (new scan, scan progress, results dashboard, finding detail), Supabase Auth (magic link), talks to `backend/api`.
...
Requires `backend/api` running locally too (defaults to `http://localhost:8000`).
...
- `src/lib/api.ts`: typed fetch client for `backend/api`, attaches the Supabase access token as a Bearer header.
